...
const getAnalyticsData = async (req, res) => {
    ...
    try {
        ...
        const levelCompletionRate = await Promise.all(courses.map(async (course) => {
...
exports.getSchoolPerformance = getSchoolPerformance;
/**
...
const sendNotification = async (req, res) => {
    ...
    try {
        ...
        switch (targetType) {
            ...
            case 'SCHOOL':
                if (!targetValue)
                    return res.status(400).json({ message: 'School value is required for this target type.' });
 ...
 */
const getDistrictSchools = async (req, res) => {
    const districtId = req.user?.districtId;
    if (!districtId) {
        return res.status(403).json({ message: 'Admin user is not associated with a district.' });
    }
    try {
        ...
        const schoolNames = schools
            .map(s => s.school)
            .filter((school) => school !== null); // TypeScript guard to remove nulls
...
exports.updateDistrictEvent = updateDistrictEvent;
